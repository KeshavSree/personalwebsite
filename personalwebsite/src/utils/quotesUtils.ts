...
// `personalwebsite/blog/posts/<slug>.md`, strips frontmatter, returns the
// markdown body (or null if the file doesn't exist). Blog posts are
// authoritative Keshav prose — the picker reads them directly instead of
// requiring a copied corpus file.
function readBlogPostBody(slug: string): string | null {
  const candidates = [
    path.join(process.cwd(), 'blog', 'posts', `${slug}.md`),
    path.join(process.cwd(), '..', 'personalwebsite', 'blog', 'posts', `${slug}.md`),

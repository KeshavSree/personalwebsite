...
async function buildHydeQuery(
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: `You generate a 1-2 sentence hypothetical answer to a question about Keshav Sreekantham, written as if you knew everything about him. The text is used only as a retrieval query (embedded against a vector store), never shown to the user.
...
- When the question uses general or category-level wording, name specific entities, places, projects, or activities you can plausibly infer about Keshav. Bridging vocabulary from general to specific is the entire point.
- Do NOT invent specific facts you'd be embarrassed to be wrong about — exact dates, company names you've never heard of, named partners. If unsure, stay topical but vague ("Keshav has worked on several research projects").
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Keshav puts in the corpus is guaranteed to be visible to the reply.
    //
    ...
    const rawEntries: Array<{ id: string; label: string }> = [
      ...
      ...[...retrievedTopics.entries()].map(([slug, { title, tagline, text }]) => ({
        id: `topic:${slug}`,
        label: `[Keshav's take on "${slug}"] ${title}${tagline ? ` (${tagline})` : ""} — ${blurb(text, 200)}`,
      })),
    ];
...
- About: https://www.keshavsreekantham.com/about
- Projects: https://www.keshavsreekantham.com/projects
- Work Experience: https://www.keshavsreekantham.com/work
- Involvement: https://www.keshavsreekantham.com/involvement
...
- Link labels are human words, never a raw URL. Prefer linking a noun already in your sentence: "browse his [Projects](https://www.keshavsreekantham.com/projects) and [Work Experience](https://www.keshavsreekantham.com/work)". Never write a link whose label is the URL (never "[https://site.com](https://site.com)"), and always close the parenthesis.`;
    ...
    if (hasRelevantContext) {
      systemPrompt = `You are Keshav's AI representative on his portfolio website (keshavsreekantham.com). You know him well and speak about him with grounded enthusiasm. Conversational and factual.
...
- Lead with the take, not the projects. The first sentence states his stance using his vocabulary, his angle, his sharpness. No hedge ("Keshav is opinionated on X"), no project intro, no definition.
...
- When you mention a project by name, the project name itself is the link. Default target: \`https://www.keshavsreekantham.com/projects#<project-id>\` where \`<project-id>\` is the slug from the Context's directory entry.
...
Context about Keshav:
${contexts}`;
    } else {
      systemPrompt = `You are Keshav's AI representative on his portfolio website (keshavsreekantham.com). The retrieval system found nothing relevant for this query, which usually means the question is off-topic, or it's about Keshav but missed the index.

${HARD_CONSTRAINTS}

DEFAULT BEHAVIOR. Decline off-topic queries per Rule 1 in one short friendly sentence and redirect. If the question is plainly about Keshav but happened to miss the index, say you don't have specifics on that topic and suggest related areas you can help with (drawn from the list below). Do not invent details to fill the gap. The Hard Constraints still apply: no fabrication, no plural padding, no training-data inference.
        ...
        try {
          ...
          if (replyText.trim().length > 20) {
            try {
              ...
              // sometimes emit paths without `keshavsreekantham.com` (just
              // `[label](/involvement#slug)`). Allow:
              //   keshavsreekantham.com/<path>
...
${corpus}`,
              ...
              const runTopicExtractor = async (): Promise<string[]> => {
                ...
                const topicDirectory = topicEntries
                  .map(
                    ([slug, t], i) =>
                      `[${i + 1}] [Keshav's take on "${slug}"] ${t.title}${t.tagline ? ` (${t.tagline})` : ""}`,
                  )
                  .join("\n");
                ...
                try {
                  const res = await openai.chat.completions.create({
                    ...
                    messages: [
                      {
                        role: "system",
                        content: `You decide which Keshav-take cards are anchored by an assistant reply. Each card represents Keshav's stance on a topic.

Return JSON: {"cited":[<index>,<index>,...]} (zero or more integers from the DIRECTORY).

Inclusion rule: TOPIC CARDS ARE THE CANONICAL HOME OF HIS STANCE. If the reply substantively states or discusses Keshav's view on the topic's subject — even abstractly, even when the reply also covers projects — INCLUDE the topic card. A topic card is NEVER "merely topically adjacent" to a reply about its subject; it IS the take.

...
PROJECTS_JSON_CANDIDATES = [
    os.path.join("..", "personalwebsite", "src", "data", "projects.json"),  # From python-rag/
    os.path.join("personalwebsite", "src", "data", "projects.json"),       # From root
]
...
def load_text_files():
    ...
    # Load corpus files — Keshav's own opinionated prose, indexed per artifact
...
def load_corpus_files():
    ...
    of Keshav's opinionated prose, with frontmatter declaring `applies_to`
...
# === BLOG POST LOADING ===
def load_blog_posts():
    """Load and process blog posts from personalwebsite/blog/posts directory"""
    blog_docs = []

    # Try multiple potential paths for the blog posts (from python-rag/ directory)
    potential_paths = [
        "../personalwebsite/blog/posts",  # From python-rag/ to personalwebsite/blog/posts
        "personalwebsite/blog/posts",      # If running from root
...
def chunk_documents(documents):
    ...
    for doc in documents:
        ...
        if source_type == 'blog':
            ...
            # Corpus files are organized by `## <Sub-topic>` headings — Keshav's
